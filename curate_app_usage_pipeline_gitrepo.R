...
curate_mars_int <- function(df, id){
  #number of pages viewed (count the num of state in V3)
  df$num_page <- rep(NA, nrow(df))
  #number of clicks made (count the num of "selected" or "pressed" in V6)
    ...
    if(nrow(mars_log_s) > 0){
      ...
      for (r in 1:nrow(df)){
        ...
        if(nrow(curr_mars) == 0){
          ...
          #curate time spent (one threshold is that it removes durations with nothing happened for more than 10mins)
...
#This is for curating quit tip interaction (prompted or self-initiated)
#Again, any repetitive codes as above (e.g., finding the log corresponding to the current decision point) are not annotated


curate_tips_int <- function(df, id){
  #count the num of messages (count messages in V3)
  ...
  #time elapsed from seeing the message to making an action
  df$tips_comp_time <- rep(NA, nrow(df))
  #word per second based on the time and message length
  ...
  if(file.exists(fname_tips_log)){
    ...
    #some logs have up to 7 columns
    ...
    if(nrow(tips_log_s) > 0){
      for (r in 1:nrow(df)){
        ...
        if(nrow(curr_tips) == 0){
          ...
          #because low-effort log also includes 2Q survey interactions, the flags below is to make sure subject is in quit tips
          ...
          #considered completed if they clicked 'close' or 'next'; both cases have 'button clicked' flag in V3
          ...
          if(closed == 1 & df$in_tips[r] == 1){
            ...
            ##Codes below are to calculate the time for completing quit tips
            time <- c()
            wps <- c()
            ...
            for (i in 1:nrow(curr_tips)){
              ...
              #in most cases, the "message shown" flag is followed immediately by "button clicked" flag
              #it's easy to find the time elapsed by calculating the difference between the timestamps of the two rows
              if(str_detect(curr_tips$V3[i], "Message shown") & 
                 str_detect(curr_tips$V3[i + 1], "button clicked")){
                
                #This is for the rare cases where the logging seems to have a bug. Usually happens when subject is viewing more than one messages very quickly
                #The "clicked" flag will be on row i + 2 rather than i + 1
            ...
            #As long as there is one message where the timing fits the criterion, it can be considered completed
            #So we get the longest time, or the smallest words per second
...
#Curating only quit tips app usage that is prompted
...
#Compiling the dataset that includes the app usage variables
...
#A few rare cases where the log file doesn't have any information on the response to intervention prompts
...
#Running the functions for curating app usage variables
for (s in 1:length(id_list)){
  ...
  curr_df_tips <- curate_tips_int(curr_df_mars_p, id_list[s])
  curr_df_tips_p <- curate_tips_int_p(curr_df_tips, id_list[s])
  curated_w_app_usage <- rbind(curated_w_app_usage, curr_df_tips_p)
}



curated_w_app_usage_to_merge <- curated_w_app_usage %>% 
  select(mars_id, decision_point, num_page:tips_comp_wps_p)


#Merge back to main dataset with all decision points
...
#Apply the cutoff of reading speed for low-effort strategy
dat_mars_app_engage$read_tips_cutoff <- ifelse(dat_mars_app_engage$read_tips == 1 & dat_mars_app_engage$tips_comp_wps > (340/60), 
                                               0, dat_mars_app_engage$read_tips)
dat_mars_app_engage$read_tips_cutoff_p <- ifelse(dat_mars_app_engage$read_tips_p == 1 & dat_mars_app_engage$tips_comp_wps_p > (340/60), 
                                                 0, dat_mars_app_engage$read_tips_p)

#Apply the cutoff of reading speed for high-effort strategy
...
#Setting values of zero for subject 15 (log file non-existent for this subject)
...
#Engagement with any strategy (both low- and high-effort)
dat_mars_app_engage$strat_done_wcutoff <- as.integer((dat_mars_app_engage$activ_done + dat_mars_app_engage$read_tips_cutoff) > 0)



saveRDS(dat_mars_app_engage, file.path(path_manipulated_data, "dat_mars_app_engage.rds"))
